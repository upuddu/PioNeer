...
public:
    ...
    #define _BLOCK_PIN(P) case P: {                             \
        ...
        FastPin<P>::setOutput();                                \
        m_bitOffsets[mNActualLanes++] = FastPin<P>::pinbit();  \
        mNWriteMask |= FastPin<P>::mask();                     \
  ...
  template<int BITS,int PX> __attribute__ ((always_inline)) FL_NOEXCEPT inline void writeBits(FASTLED_REGISTER u32 & next_mark, FASTLED_REGISTER _outlines & b, PixelController<RGB_ORDER, LANES, __FL_T4_MASK> &pixels) {
        _outlines b2;
        ...
        for(u32 i = 8; i > 0;) {
            ...
            *FastPin<FIRST_PIN>::sport() = mNWriteMask;
...
template<template<u8 DATA_PIN, EOrder RGB_ORDER> class CHIPSET, u8 DATA_PIN, int NUM_LANES, EOrder RGB_ORDER=GRB>
