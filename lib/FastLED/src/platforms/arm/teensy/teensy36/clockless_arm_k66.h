...
protected:
	...
	template<int BITS> __attribute__ ((always_inline)) FL_NOEXCEPT inline static void writeBits(FASTLED_REGISTER u32 & next_mark, FASTLED_REGISTER data_ptr_t port, FASTLED_REGISTER data_t hi, FASTLED_REGISTER data_t lo, FASTLED_REGISTER u8 & b)  {
	...
	static u32 showRGBInternal(PixelController<RGB_ORDER> pixels, Rgbw rgbw) FL_NOEXCEPT {
		...
		*port = lo;
		...
		while(pixels.has(1)) {
			...
			#if (FASTLED_ALLOW_INTERRUPTS == 1)
			// Only call cli() after the first pixel, since line 84 already disabled interrupts initially
			if (!first_pixel) {
				cli();
			}
			first_pixel = false;
