/*
  FastLED — RP2040 ISR Implementation
  ------------------------------------
  Platform-specific ISR implementation for RP2040 (Raspberry Pi Pico).

  RP2040 Hardware:
  - Dual Cortex-M0+ cores (multi-core capable)
  ...
  - NVIC priority system (lower number = higher priority, 0-3 range on M0+)
...
// Platform ID for RP2040
constexpr u8 RP2040_PLATFORM_ID = 4;
...
static u8 map_priority_to_nvic(u8 isr_priority) FL_NOEXCEPT {
    ...
    // Map ISR priorities 1-7 to NVIC priorities 3-0
    // ISR 1-2 -> NVIC 3 (lowest)
    // ISR 3-4 -> NVIC 2
    // ISR 5-6 -> NVIC 1
    // ISR 7   -> NVIC 0 (highest)
    if (isr_priority <= 2) return 3;
    if (isr_priority <= 4) return 2;
    if (isr_priority <= 6) return 1;
    return 0;
}
...
int attach_timer_handler(const isr_config_t& config, isr_handle_t* out_handle) FL_NOEXCEPT {
    ...
    if (out_handle) {
        ...
        out_handle->platform_id = RP2040_PLATFORM_ID;
    }
