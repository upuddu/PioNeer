...
///     // PARLIO driver auto-selects 1/2/4/8/16-bit mode based on channel count
/// }
///
/// void loop() {
///     // Update LED data
...
/// The driver automatically selects optimal data width per batch:
///
/// | Channels | Selected Engine | Data Width | Dummy Lanes | Memory per 100
/// LEDs |
/// |----------|-----------------|------------|-------------|---------------------|
/// | 1        | 1-bit driver    | 1-bit      | 0           | ~1.2 KB | | 2 |
/// 2-bit driver    | 2-bit      | 0           | ~2.4 KB             | | 3-4 |
/// 4-bit driver    | 4-bit      | 1-0         | ~4.8 KB             | | 5-8 |
/// 8-bit driver    | 8-bit      | 3-0         | ~9.6 KB             | | 9-16 |
/// 16-bit driver   | 16-bit     | 7-0         | ~19.2 KB            |
///
...
/// - Maximum capability allocated at construction (all 1/2/4/8/16-bit drivers)
///
/// ## Limitations
...
///
/// ## See Also
/// - Implementation: `channel_driver_parlio.cpp`
...
class ChannelDriverPARLIOImpl : public IChannelDriver {
  ...
  private:
    ...
    /// @brief HAL driver reference (singleton)
    detail::ParlioEngine& mDriver;
    ...
    /// @brief Scratch buffer for per-lane data layout (owned by channel driver)
...
/// @brief PARLIO driver with lazy initialization and dynamic reconfiguration
///
...
///   width
/// - Inherits from IChannelDriver for standard FastLED integration
...
///    - If width needs to change: delete driver, create new one
/// 4. **Polling**: poll() checks driver state (if initialized)
/// 5. **Destruction**: Cleans up driver instance
///
/// ## Memory Management (Single Engine)
/// - Only ONE driver instance allocated at a time
...
/// @brief Create PARLIO driver instance with lazy initialization
