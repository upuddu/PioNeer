...
class u8x24 {
  public:
    ...
    explicit constexpr u8x24(float f) FL_NOEXCEPT
    ...
    explicit constexpr u8x24(IntT n) FL_NOEXCEPT
 ...
 FL_NOEXCEPT : mValue(static_cast<u32>(
    ...
    struct RawTag {};
    constexpr explicit u8x24(u32 raw, RawTag) FL_NOEXCEPT : mValue(raw) {}

    static constexpr FASTLED_FORCE_INLINE u8x24 from_raw(u32 raw) FL_NOEXCEPT {
        return u8x24(raw, RawTag());
    }
    ...
    constexpr bool operator<(u8x24 b) const FL_NOEXCEPT { return mValue < b.mValue; }
    constexpr bool operator>(u8x24 b) const FL_NOEXCEPT { return mValue > b.mValue; }
    constexpr bool operator<=(u8x24 b) const FL_NOEXCEPT { return mValue <= b.mValue; }
    constexpr bool operator>=(u8x24 b) const FL_NOEXCEPT { return mValue >= b.mValue; }
    constexpr bool operator==(u8x24 b) const FL_NOEXCEPT { return mValue == b.mValue; }
    ...
    static constexpr FASTLED_FORCE_INLINE u8x24 step(u8x24 edge, u8x24 x) FL_NOEXCEPT {
        return x < edge ? u8x24() : u8x24(1.0f);
    }
    ...
    static constexpr FASTLED_FORCE_INLINE u8x24 sqrt(u8x24 x) FL_NOEXCEPT {
        return x.mValue == 0 ? u8x24() : from_raw(static_cast<u32>(
