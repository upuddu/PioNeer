...
{
    if (index >= WS2812_NUM_LEDS || color >= COLOR_COUNT)
        return;
...
{
    if (index >= WS2812_NUM_LEDS || !r || !g || !b)
        return;
    *r = leds[index].r;
    *g = leds[index].g;
    *b = leds[index].b;
...
{
    if (from_index >= WS2812_NUM_LEDS || to_index >= WS2812_NUM_LEDS)
        return;
...
{
    if (index1 >= WS2812_NUM_LEDS || index2 >= WS2812_NUM_LEDS)
        return;
...
{
    if (color >= COLOR_COUNT || spacing == 0)
        return;
    rgb_t c = color_table[color];
...
{
    if (color1 >= COLOR_COUNT || color2 >= COLOR_COUNT)
        return;
...
void led_strip_set_segment(uint8_t start, uint8_t end, uint8_t r, uint8_t g, uint8_t b)
{
    if (start >= WS2812_NUM_LEDS)
        return;
    if (end >= WS2812_NUM_LEDS)
        end = WS2812_NUM_LEDS - 1;
    for (uint8_t i = start; i <= end; i++)
    {
        leds[i].r = r;
        leds[i].g = g;
